...
(function () {
  ...
  styleEl.textContent = `
    ...
    #sofia-launcher { position: fixed; bottom: 24px; right: 24px; width: 60px; height: 60px; border-radius: 50%; background: var(--accent); border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 9998; box-shadow: 0 4px 14px rgba(0,0,0,0.18), 0 0 0 0 rgba(232,115,95,0.55); transition: transform 0.15s ease; animation: sofia-glow-intro 1.8s ease-out 3, sofia-glow-idle 4.5s ease-in-out 5.4s infinite; }
    #sofia-launcher:hover { transform: scale(1.06); }
    #sofia-launcher .icon-close { display: none; }
    body.sofia-open #sofia-launcher { animation: none; box-shadow: 0 4px 14px rgba(0,0,0,0.18); }
    body.sofia-open #sofia-launcher .icon-chat { display: none; }
    body.sofia-open #sofia-launcher .icon-close { display: block; }
    @keyframes sofia-glow-intro { 0% { box-shadow: 0 4px 14px rgba(0,0,0,0.18), 0 0 0 0 rgba(232,115,95,0.55); } 60% { box-shadow: 0 4px 14px rgba(0,0,0,0.18), 0 0 0 14px rgba(232,115,95,0); } 100% { box-shadow: 0 4px 14px rgba(0,0,0,0.18), 0 0 0 0 rgba(232,115,95,0); } }
    @keyframes sofia-glow-idle { 0%, 100% { box-shadow: 0 4px 14px rgba(0,0,0,0.18), 0 0 8px 1px rgba(232,115,95,0.22); } 50% { box-shadow: 0 4px 14px rgba(0,0,0,0.18), 0 0 15px 3px rgba(232,115,95,0.42); } }
    @media (prefers-reduced-motion: reduce) { #sofia-launcher { animation: none; box-shadow: 0 4px 14px rgba(0,0,0,0.18), 0 0 10px 2px rgba(232,115,95,0.3); } }
